...
(function(window, document, callback) { // http://stackoverflow.com/questions/2170439/how-to-embed-javascript-widget-that-depends-on-jquery-into-an-unknown-environmen
    ...
    var SOUNDCITE_CONFIG = {
        update_playing_element: function(el, percentage) {
             el.style.cssText =
                'background: -webkit-linear-gradient(left, rgba(255,255,255,0.75)' + percentage + '%, rgba(255,255,255,0.25)' + (percentage + 1) + '%);'
              + 'background: linear-gradient(to right, rgba(255,255,255,0.75)' + percentage + '%, rgba(255,255,255,0.25)' + (percentage + 1) + '%);'
        },
